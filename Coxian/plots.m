...
%This script compares the stationary distributions obtained using the
%matrix-analytic method, constant diffusion coefficient approximation, and
%state-dependent coefficient approximation. Can modify to obtain any
...
for count = 1:length(nvals)
    n = nvals(count);
    f1 = strcat('C2yc_n', int2str(n) , '_rho08_cs24');
    f2 = strcat('C2ysd_n', int2str(n) , '_rho08_cs24');
    f3 = strcat('maC2_n_', int2str(n) , '_rho_08_cs_24');
    load (f1);
    load (f2);
    load (f3);
